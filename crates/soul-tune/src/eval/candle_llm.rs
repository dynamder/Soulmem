...
fn remap_metadata(content: &mut gguf_file::Content, target_prefix: &[&str]) {
    ...
    // Provide defaults and compute missing keys for all target prefixes
    for tp in target_prefix {
...
impl CandleLlm {
    pub fn load(config: CandleLlmConfig) -> Result<Self> {
        ...
        // First pass: detect architecture
        ...
        // Second pass: load with backend matching architecture
    ...
    pub fn chat(&mut self, system: &str, user: &str) -> Result<String> {
        let prompt = format!(
            "<|im_start|>system\n{}<|im_end|>\n<|im_start|>user\n{}<|im_end|>\n<|im_start|>assistant\n",
            system, user
        );
    ...
    pub fn generate(&mut self, prompt: &str) -> Result<String> {
        ...
        // Process all prompt tokens to populate KV cache
    ...
    pub fn generate_queries(&mut self, system: &str, user_message: &str) -> Result<String> {
        let prompt = format!(
