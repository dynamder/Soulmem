...
use crate::eval::batch::{
    process_one_compare_dataset, process_one_dataset, scan_question_jsons, BatchCompareResult,
    BatchResult, CompareDatasetResult, DatasetResult,
};
...
impl BatchRunState {
    ...
    fn render_compare_table(&self, frame: &mut Frame, area: Rect, result: &BatchCompareResult) {
        ...
        let (cr, bar_rect) = ScrollContainer::split_area(inner);
        ...
        let so = ScrollContainer::offset(visible, result.datasets.len(), self.table_scroll.cursor);
        ...
        ScrollContainer::render_scrollbar(frame, bar_rect, total_lines, visible, scroll_ylines);
    }
    ...
    fn render_dataset_table(&self, frame: &mut Frame, area: Rect, result: &BatchResult) {
        ...
        let (cr, br) = ScrollContainer::split_area(inner);
        let so = ScrollContainer::offset(
