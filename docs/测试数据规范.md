...
不写 `blend_sweep` 字段 = 只执行一次 `(tag=0.4, variant=0.6)` 的默认权重。
...
## 六、仓库中的测试数据：来源、许可与入库分层

第一到第五节描述的是**格式**。这一节描述的是**这些东西为什么可以放在这个仓库里、以及哪些可以放**。

### 6.1 数据来源链

`fixtures/example_data/` 下的每套角色数据都是合成数据，但不是凭空生成的：

```
萌娘百科角色页面
  └── scrape.md      LLM 以角色第一人称视角转述该页面的「研究报告」
        ├── graph.json        由该报告合成的记忆图
        └── question.json     由该图派生的检索测试用例
```

来源 URL 编码在目录名里，例如 `丛雨_https_zh_moegirl_org_cn_E4_B8_9B_E9_9B_A8/`：
`丛雨` 是角色名，`https_zh_moegirl_org_cn` 是站点，其后的 `_XX` 是页面标题的 UTF-8 字节十六进制，
解码即 `https://zh.moegirl.org.cn/丛雨`。

因此 `scrape.md` 与 `graph.json` 是**上游页面内容的演绎作品**，不是独立创作的原始数据。

### 6.2 许可：上游条款与本项目用途不冲突

萌娘百科的[著作权信息](https://zh.moegirl.org.cn/%E8%90%8C%E5%A8%98%E7%99%BE%E7%A7%91:%E8%91%97%E4%BD%9C%E6%9D%83%E4%BF%A1%E6%81%AF)页面写明：

> 文本内容除特殊注明外，均在**知识共享 署名-非商业性使用-相同方式共享 3.0 中国大陆
> （CC BY-NC-SA 3.0 CN）许可协议**下提供，附加条款亦可能应用。

**SoulMem 是非商业化的开源项目**（个人自用 + 公开源码，不做商业化运营）。因此上游许可里最受
关注的那条——**NC（非商业性使用）——对本项目的使用方式不构成阻碍，不需要为此替换数据**。

仍然实际成立的是另外两条，记录下来即可，都不需要采取行动：

1. **SA（相同方式共享）**：`fixtures/example_data/` 里的演绎作品继续以 CC BY-NC-SA 3.0 CN 提供，
   不由仓库的 MIT 覆盖。这不是问题，只是别把这部分内容当成 MIT 内容即可。
2. **署名**：转载时"必须给出原页面的 URL 链接，并注明……从萌娘百科转载"。当前来源 URL 只是
   **编码在目录名里**，既不直观也不构成链接，严格说这不满足署名要求（见 6.4）。

**唯一需要留意的边界**：NC 约束的是**使用方**，而不是本项目。第三方如果把 SoulMem 用于商业目的，
仓库代码（MIT）没问题，但 `fixtures/example_data/` 这部分数据不可用。所以在 `README.md` 里用一句话
说明这批数据的来源与限制是**值得做**的——但它不是合入阻塞项。

### 6.3 入库分层

| 层 | 内容 | 是否入库 | 理由 |
|---|---|---|---|
| 一 | 小型、手工合成的测试数据：`graph.json`、`question.json`、`scrape.md`、`daily_dialogues/*.json`、`queries/*`、`graphs/*`（当前 **171 文件 / 4.1 MB**） | ✅ 必须入库 | 测试契约。没有它 `cargo test` 与 soul-tune 实验都不可复现 |
| 二 | 派生产物：`*.embcache`（当前 **24 文件 / 约 48 MB**）、SurrealDB 库文件、下载的模型、实验输出 | ❌ 不入库 | 可由脚本确定性重生成；`.embcache` 还只按版本号失效、不随图边重建，入库会让人读到旧数据 |
| 三 | 大数据集（未来）：单文件 > 1 MB，或单个数据集 > 10 MB，或可由脚本 + 种子重生成 | ❌ 出仓 | git 主历史会永久保留每一次改动，改十次就是十份。改为发布为 release asset / 独立数据仓库，配 `scripts/fetch_*.py` + SHA256 校验 |

一句话判据：**能由脚本确定性地重生成 → 出仓；是测试契约的一部分 → 入库。**

**已知的不一致（需要一次决定）**：根 `.gitignore` 忽略了 `graph_stats.json` 与 `raw_failed_*.json`，
但库里已经有 **24 个 `graph_stats.json` 和 26 个 `raw_failed_*.json`** 处于入库状态
（`.gitignore` 不影响已跟踪文件）。结果是同一类文件"旧的入库、新的被忽略"。要么把这 50 个文件删掉
（它们确实是派生产物），要么从 `.gitignore` 中删掉这两条并明确它们该入库，不要维持现状。

### 6.4 TODO

- [ ] （建议，非阻塞）**在 `README.md`（及 `README_en.md`）用一句话说明** `fixtures/example_data/`
      来自萌娘百科、按其 CC BY-NC-SA 3.0 CN 条款提供，以免将本项目用于商业目的的第三方误用这部分数据。
- [ ] **补可用的署名**：新建 `fixtures/example_data/NOTICE.md`，为每个角色目录列出角色名与原页面 URL
      （按 6.1 的编码规则解码即可），并声明该目录整体按 CC BY-NC-SA 3.0 CN 提供。
      只把 URL 编码在目录名里不构成"给出原页面 URL 链接"。
- [ ] **记录合成方式**：`scrape.md` 与 `graph.json` 由哪个工具 / 提示词 / 模型版本生成。
      没有这一步，"这批数据用于测试整个框架效果"对贡献者是不可复现的。
