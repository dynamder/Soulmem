...
1. **NC（非商业性使用）**：本仓库代码是 MIT，可以商用；但 `fixtures/example_data/` 里的演绎内容
   **不可以**。这条必须写进 `README.md` 的许可证章节，否则使用者会误判。
2. **SA（相同方式共享）**：这些演绎作品必须继续以 CC BY-NC-SA 3.0 CN 提供，不能被 MIT 重新授权。
3. **署名**：转载时"必须给出原页面的 URL 链接，并注明……从萌娘百科转载"。当前来源 URL 只是
...
- [ ] **在 `README.md`（以及 `README_en.md`）的许可证章节补一条例外**：`fixtures/example_data/`
      中的 `scrape.md` / `graph.json` 是萌娘百科文本（CC BY-NC-SA 3.0 CN）的演绎作品，
      不适用本仓库的 MIT，且不得商业使用。
...
- [ ] （可选，但推荐）**把 `fixtures/example_data/` 换成完全自造的数据**：这样整套 fixtures 落在
      MIT 之下，上面三条同时消失，仓库也就可以整体被商业使用。
