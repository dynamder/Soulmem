...
SoulMem 以图的形式组织记忆。图上的节点被称为记忆节点，边被称为记忆关联。用图的方式组织记忆，联想与多跳推理会变得自然。
然而，SoulMem 并不只维护一个图，组织记忆的也不只有图这一种形式，这一点将在后续章节详细说明。
...
> *注*：SoulMem 代码库中采用 Situation 的命名；它对应认知科学中的情景记忆（Episodic Memory），两者是同一回事。
...
事实上，角色的自我认知也是一个语义记忆节点，它的内容与常规角色扮演应用中的**角色卡**十分类似。这样的设计使角色的自我进化成为可能，具体机制将在后续章节详细说明。
